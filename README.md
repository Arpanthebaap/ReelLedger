...
## Google Cloud & ClickHouse Cloud Monitoring & Deployment

ReelLedger connects to a live, scalable ClickHouse Cloud cluster and runs on Google Cloud Run with Vertex AI integration.

### ClickHouse Cloud Monitoring
- **Cluster Overview**: Scaled to 16 GiB RAM, 4 vCPU per replica to handle concurrent agent analytical queries.
  ![ClickHouse Cloud Overview](docs/images/clickhouse_overview_details.png)
  
- **Infrastructure Scaling**: Built-in horizontal replica scale out and active replica monitoring.
  ![ClickHouse Cloud Infrastructure](docs/images/clickhouse_infrastructure.png)
  
- **Query Insights**: High-performance columnar aggregation of multi-agent queries with sub-millisecond response latency.
  ![ClickHouse Cloud Query Insights](docs/images/clickhouse_query_insights.png)

### Google Cloud Platform Monitoring
- **Cloud Run Performance**: High-performance container response times and request routing metrics showing cold start warmup configurations.
  ![Google Cloud Run Monitoring](docs/images/gcp_cloud_run_monitoring.png)
  
- **Console Dashboards**: Configured Google Cloud Stackdriver dashboards monitoring container resources, log-based metrics, and IAM API endpoints.
  ![Google Cloud Console Dashboards](docs/images/gcp_dashboards_list.png)
  
- **Audit Logging**: Trace log history showing successful builds, IAM service account role allocations (`Vertex AI User` and `Secret Manager Secret Accessor`), and deployment pipelines.
  ![Google Cloud Console Audit Logging](docs/images/gcp_logs_explorer_deploy.png)
